...
void main() {
  group('tryAsyncMap detailed tests', () {
    ...
    test('handles error in chain of async operations', () async {
      ...
      final error = result.errorOrNull;
      expect(error, isNotNull);
      expect(error!.title, equals('Error'));
      expect(error.msm, contains('Failed to fetch user details'));
    });
  ...
  group('Timeout and error handling scenarios', () {
    test('handles timed out operations with fallbacks', () async {
      ...
      if (timeoutResult.isOk) {
        ...
        if (timeoutResult.errorOrNull?.title == 'Timeout') {
          // Use fallback data
    ...
    test('handles multiple concurrent async operations', () async {
      ...
      expect(results[2].errorOrNull?.title, equals('Server Error'));
        ...
        switch (errorType) {
          case 'dns':
            return Err(
              ApiErr(
                title: 'DNS Error',
                msm: 'Could not resolve host name',
          ...
          case 'timeout':
            return Err(
              ApiErr(
                title: 'Timeout',
                msm: 'Request timed out after 5 seconds',
  ...
  group('Performance and load tests', () {
    test('handles large number of sequential async operations', () async {
        ...
        if (number % 10 == 0) {
          ...
          return Err(
            ApiErr(
              ...
              msm: 'Cannot process multiple of 10: $number',
            ),
          );
        }
        return Ok(number * 2);
      ...
      expect(results[9].errorOrNull?.msm, contains('10'));
