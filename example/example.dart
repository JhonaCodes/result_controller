...
void apiResultExamples() {
  ...
  usersResult.when(
    ok: (users) => log('  Created ${users.length} users: ${users.map((u) => u.name).join(', ')}'),
    err: (error) => log('  Error: ${error.msm}'),
  );
  ...
  errorResult.when(
    ...
    err: (error) => log('  ${error.title}: ${error.msm} (Status: ${errorResult.statusCode})'),
  );
}
...
void advancedPatterns() {
  ...
  final recovered = parseNumber('invalid')
  ...
  final errors = results.where((r) => r.isErr).map((r) => r.errorOrNull).toList();
  ...
  final mappedError = parseNumber('invalid')
